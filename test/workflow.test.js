...
for (const scenario of ['success', 'shared-success', 'shared-list-only', 'conversion-error', 'retryable-error', 'upload-error', 'missing-tool']) {
...
fs.writeFileSync(args.at(-1),'generated-media');`;
        ...
        let handled = false; const calls = []; const apis = []; let upload = '';
        const server = http.createServer(async (req, res) => {
            ...
            apis.push(params.get('api') || form.get('api') || body.match(/name="api"\r\n\r\n([^\r]+)/)?.[1]);
            let data = {};
        ...
        try {
            ...
            fs.writeFileSync(configPath, JSON.stringify({ accounts: [{ url: `http://127.0.0.1:${server.address().port}`, username: 'test', password: 'test', ...(scenario.startsWith('shared-') ? { space: 'shared' } : {}) }] }));
            const child = spawn(process.execPath, [path.resolve(__dirname, '../main.js')], {
                cwd: dir, env: { ...process.env, LIST_ONLY: String(scenario === 'shared-list-only'), MAX_FILES: '1', PATH: bin, CONFIG_PATH: configPath, TEMP_DIR: path.join(dir, 'tmp'), VIDEO_BACKEND: 'software', EXIT_ON_FAIL: String(scenario === 'retryable-error'), TEST_FAIL: String(['conversion-error', 'retryable-error'].includes(scenario)) },
            });
            let output = ''; child.stdout.on('data', b => output += b); child.stderr.on('data', b => output += b);
            const code = await new Promise((resolve, reject) => { child.on('error', reject); child.on('close', resolve); });
            assert.equal(code, ['success', 'shared-success', 'shared-list-only'].includes(scenario) ? 0 : 1, output);
            ...
            if (scenario === 'shared-list-only') {
                assert.deepEqual(calls, ['login', 'list_convert_needed']);
                assert.ok(apis.includes('SYNO.FotoTeam.Upload.ConvertedFile'));
            }
            if (['success', 'shared-success'].includes(scenario)) {
                ...
                const prefix = scenario === 'shared-success' ? 'SYNO.FotoTeam' : 'SYNO.Foto';
                assert.ok(apis.includes(prefix + '.Download'));
                assert.ok(apis.filter(a => a === prefix + '.Upload.ConvertedFile').length >= 2);
            }
        } finally {
